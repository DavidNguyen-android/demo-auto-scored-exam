...
class CNN_Model(object):
    ...
    def load_data():
        ...
        for img_path in Path(dataset_dir + 'unchoice/').glob("*.png"):
            ...
            label = conv_utils(0, num_classes=2)
        ...
        for img_path in Path(dataset_dir + 'choice/').glob("*.png"):
            ...
            img = cv2.resize(img, (28, 28), cv2.INTER_AREA)
            img = img.reshape((28, 28, 1))
            label = conv_utils(1, num_classes=2)
            images.append(img / 255.0)
            labels.append(label)
    ...
    def train(self):
        ...
        # Model Checkpoint
        cpt_save = ModelCheckpoint('D:\python-demo\Auto-Scores-National-Multiple-Choice-Test-master\weight.h5', save_best_only=True, monitor='val_acc', mode='max')
